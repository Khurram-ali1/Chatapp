...
const Chatbot = () => {
    ...
    const hasFetchedCountry = useRef(false);
    ...
    const useVisitorTracking = () => {
      ...
      useEffect(() => {
        ...
        if (!hasFetchedCountry.current) {
          hasFetchedCountry.current = true;
          const fetchCountry = async () => {
            try {
              const res = await fetch("https://api.country.is/");
              const data = await res.json();
              setVisitorData((prev) => {
                const updatedData = { ...prev, country: data.country };
        ...
        const trackPageVisit = (pageURL) => {
          ...
          setVisitorData((prev) => {
            if (!prev.visitedPages.some((entry) => entry.page === pageURL)) {
              const updatedPages = [...prev.visitedPages, { page: pageURL, time: now }];
              const updatedData = { ...prev, visitedPages: updatedPages };
              localStorage.setItem("visitorData", JSON.stringify(updatedData));
              return updatedData;
            }
            return prev;
    ...
    const handleSend = () => {
      ...
      setInput("");
      setTimeout(() => {
        setMessages((prev) => [...prev, { sender: "bot", text: getBotResponse(input.trim()) }]);
      }, 1000);
    };
    ...
    const getBotResponse = (message) => {
      ...
      return responses[message.trim().toLowerCase()] || "I'm not sure how to respond to that.";
    };
